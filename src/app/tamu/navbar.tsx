...
const Navbar: React.FC = () => {
    ...
    const MenuItem = ({ href, Icon, label }: { href: string; Icon: React.ComponentType; label: string }) => (
        <li>
            <Link href={href} className={`flex items-center p-2 rounded-lg ${currentPath === href ? 'bg-indigo-100 dark:bg-indigo-950 text-indigo-600 dark:text-indigo-400' : 'text-gray-900 dark:text-white'} transition-all hover:bg-indigo-50 dark:hover:bg-indigo-900 hover:text-indigo-400 dark:hover:text-indigo-200 group`}>
                <Icon />
                <span>{label}</span>
            </Link>
        </li>
    ...
    return (
        <div className="bg-white dark:bg-zinc-950 w-full mx-auto">
            <div className="fixed top-0 z-50 w-full bg-white dark:bg-zinc-950 transition-colors">
                ...
                <aside id="separator-sidebar" className={`fixed top-0 left-0 z-40 w-64 sm:w-44 md:w-56 xl:w-64 h-screen transition-transform ${isOpen ? 'translate-x-0' : '-translate-x-full'} sm:translate-x-0`} aria-label="Sidebar">
                    <div className="flex flex-col justify-between items-stretch h-full px-3 py-4 overflow-y-auto bg-white dark:bg-zinc-950 border-r dark:border-gray-700">
                        <Link href="/" className="flex items-center gap-2 ps-2.5">
                            ...
                            <p className='mt-8 sm:m-0 font-bold text-xl bg-clip-text text-transparent dark:text-white bg-[linear-gradient(107deg,#802696_8.32%,#6348CF_60.18%,#5DAEDB_105.75%)]'>
                                SIGMA
                            </p>
                        </Link>

                        <ul className="space-y-2 font-bold mt-5 sm:mt-0">
                            <p className='font-bold navbar-title text-xs mb-2'>MAIN MENU</p>
                        ...
                        <ul className="pt-4 space-y-2 font-bold border-t border-gray-200 dark:border-gray-700">
